...
@Component({
    selector: 'jhi-education-manage',
    templateUrl: './education-manage.component.html'
})
export class EducationManageComponent implements OnInit, OnDestroy {

currentAccount: any;
    educations: EducationManage[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/education-manage'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/education-manage', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EducationManage) {
        return item.id;
    }

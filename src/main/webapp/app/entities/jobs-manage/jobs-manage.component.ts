...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/jobs-manage'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: JobsManage) {
        return item.id;
    }
    registerChangeInJobs() {
        this.eventSubscriber = this.eventManager.subscribe('jobsListModification', (response) => this.loadAll());
    }
